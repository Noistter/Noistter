class HomeController < ApplicationController
    ...
    for tweet in @taux      
        ...
        if tweet.retweet_count!=0 || tweet.favorites_count!=0 || @respuestas!=0
          if tweet.retweeted_status.nil?
            @tpuntuacion[@i]=ApplicationHelper::Tpuntuado.new(tweet.id, ((@respuestas*3)+(tweet.retweet_count*2)+tweet.favorites_count)/ Math.log(Math.sqrt(tweet.user.followers_count)), tweet.user.username, auto_link(tweet.text.dup.force_encoding("UTF-8"), target: '_blank') ,tweet.urls[0]['url'], tweet.user.profile_image_url, tweet.favorite_count, tweet.retweet_count, @respuestas, "", tweet.retweeted, tweet.favorited)
            else
  ...
  def evaluarPerfil
    ...
    for tweet in @taux      
      ...
      if tweet.retweet_count!=0 || tweet.favorites_count!=0 || @respuestas!=0
          if tweet.retweeted_status.nil?
            @tpuntuacion[@i]=ApplicationHelper::Tpuntuado.new(tweet.id, ((@respuestas*3)+(tweet.retweet_count*2)+tweet.favorites_count)*100/ Math.sqrt(tweet.user.followers_count*5), tweet.user.username, auto_link(tweet.text.dup.force_encoding("UTF-8"), target: '_blank') ,tweet.urls[0]['url'], tweet.user.profile_image_url, tweet.favorite_count, tweet.retweet_count, @respuestas, "", tweet.retweeted, tweet.favorited)
            else
